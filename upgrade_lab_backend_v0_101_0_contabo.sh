...
TARGET_KEY="lab"
PRODUCT="Lab"
ARCHIVE="${1:-/tmp/sustainable-catalyst-lab-backend-v0.101.0.zip}"
ROOT="${SC_TARGET_ROOT:-/opt/sustainable-catalyst/lab}"
LIVE_BACKEND="$ROOT/backend"
COMPOSE="${SC_TARGET_COMPOSE:-$ROOT/compose.yml}"
SERVICE="${SC_TARGET_SERVICE:-lab}"
CONTAINER="${SC_TARGET_CONTAINER:-sc-lab}"
BACKUP_ROOT="/opt/sustainable-catalyst/backups"
TMP="$(mktemp -d /tmp/sc-energy-consumer-lab.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
tar -C "$ROOT" -czf "$BACKUP_ROOT/lab-backend-before-v$VERSION-$stamp.tgz" backend
cp -a "$COMPOSE" "$BACKUP_ROOT/lab-compose-before-v$VERSION-$stamp.yml"
...
f=framework()
assert f['consumer_version']=='0.101.0', f
assert f['target_key']=='lab', f
...
print('PASS: Lab v0.101.0 Energy Systems contract intake is active')
