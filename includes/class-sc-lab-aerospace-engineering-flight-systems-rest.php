<?php
/**
 * Aerospace Engineering and Flight Systems REST proxy.
 *
...
final class SC_Lab_Aerospace_Engineering_Flight_Systems_REST {
    ...
    private static function proxy(
        ...
        if ($backend === '') {
            return new WP_Error(
                'sc_lab_aerospace_backend_unavailable',
        ...
        if (!is_array($decoded)) {
            return new WP_Error(
                'sc_lab_aerospace_backend_response',
        ...
        if ($status < 200 || $status >= 300) {
            return new WP_Error(
                'sc_lab_aerospace_backend_error',
    ...
    public static function methods() {
        return self::proxy('/v1/aerospace-flight/methods');
    }
    ...
    public static function run($request) {
        ...
        if (!is_array($body)) {
            return new WP_Error(
                'sc_lab_aerospace_invalid_payload',
        ...
        if ($method_id === '' || strpos($method_id, 'af.') !== 0) {
            return new WP_Error(
                'sc_lab_aerospace_invalid_method',
                'A methodId beginning with af. is required.',
                array('status' => 422)
            );
        }
        ...
        foreach ($raw_inputs as $key => $value) {
            if (!is_numeric($value)) {
                return new WP_Error(
                    'sc_lab_aerospace_invalid_input',
                    'Aerospace and flight-system inputs must be numeric.',
                    array('status' => 422)
                );
            }
...
SC_Lab_Aerospace_Engineering_Flight_Systems_REST::boot();
