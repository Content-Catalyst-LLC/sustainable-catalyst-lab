<?php
/**
 * Rocket Propulsion and Spaceflight REST proxy.
 *
...
final class SC_Lab_Rocket_Propulsion_Spaceflight_REST {
    ...
    private static function proxy(
        ...
        if ($backend === '') {
            return new WP_Error(
                'sc_lab_rocket_spaceflight_backend_unavailable',
        ...
        if (!is_array($decoded)) {
            return new WP_Error(
                'sc_lab_rocket_spaceflight_backend_response',
        ...
        if ($status < 200 || $status >= 300) {
            return new WP_Error(
                'sc_lab_rocket_spaceflight_backend_error',
    ...
    public static function methods() {
        return self::proxy('/v1/rocket-spaceflight/methods');
    }
    ...
    public static function run($request) {
        ...
        if (!is_array($body)) {
            return new WP_Error(
                'sc_lab_rocket_spaceflight_invalid_payload',
        ...
        if ($method_id === '' || strpos($method_id, 'rp.') !== 0) {
            return new WP_Error(
                'sc_lab_rocket_spaceflight_invalid_method',
                'A methodId beginning with rp. is required.',
                array('status' => 422)
            );
        }
        ...
        foreach ($raw_inputs as $key => $value) {
            if (!is_numeric($value)) {
                return new WP_Error(
                    'sc_lab_rocket_spaceflight_invalid_input',
                    'Rocket and spaceflight inputs must be numeric.',
                    array('status' => 422)
                );
            }
...
SC_Lab_Rocket_Propulsion_Spaceflight_REST::boot();
