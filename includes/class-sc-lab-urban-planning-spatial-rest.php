<?php
/**
 * Urban Planning and Spatial Systems REST proxy.
 *
...
final class SC_Lab_Urban_Planning_Spatial_REST {
    ...
    /**
     * Resolve the server-side backend key.
     *
     * @return string
     */
    ...
    /**
     * Proxy a request to the optional FastAPI service.
     *
    ...
    private static function proxy(
        ...
        if ($backend === '') {
            return new WP_Error(
                'sc_lab_urban_backend_unavailable',
    ...
    /**
     * Return the urban-planning method catalog.
     *
     * @return mixed
     */
    public static function methods() {
        return self::proxy('/v1/urban-planning/methods');
    }

    /**
     * Run one urban-planning or spatial method.
     *
    ...
    public static function run($request) {
        ...
        if ($method_id === '' || strpos($method_id, 'up.') !== 0) {
            return new WP_Error(
                'sc_lab_urban_invalid_method',
                'A methodId beginning with up. is required.',
                array('status' => 422)
            );
        }
        ...
        foreach ($raw_inputs as $key => $value) {
            if (!is_numeric($value)) {
                return new WP_Error(
                    'sc_lab_urban_invalid_input',
                    'Urban-planning and spatial inputs must be numeric.',
                    array('status' => 422)
                );
            }
...
SC_Lab_Urban_Planning_Spatial_REST::boot();
