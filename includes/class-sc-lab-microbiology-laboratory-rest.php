<?php
/**
 * Microbiology Laboratory REST proxy.
 *
...
final class SC_Lab_Microbiology_Laboratory_REST {
    ...
    private static function proxy(
        ...
        if ($backend === '') {
            return new WP_Error(
                'sc_lab_microbiology_backend_unavailable',
        ...
        if (!is_array($decoded)) {
            return new WP_Error(
                'sc_lab_microbiology_backend_response',
        ...
        if ($status < 200 || $status >= 300) {
            return new WP_Error(
                'sc_lab_microbiology_backend_error',
    ...
    public static function methods() {
        return self::proxy('/v1/microbiology/methods');
    }
    ...
    public static function run($request) {
        ...
        if (!is_array($body)) {
            return new WP_Error(
                'sc_lab_microbiology_invalid_payload',
        ...
        if ($method_id === '' || strpos($method_id, 'mb.') !== 0) {
            return new WP_Error(
                'sc_lab_microbiology_invalid_method',
                'A methodId beginning with mb. is required.',
                array('status' => 422)
            );
        }
        ...
        foreach ($raw_inputs as $key => $value) {
            if (!is_numeric($value)) {
                return new WP_Error(
                    'sc_lab_microbiology_invalid_input',
                    'Microbiology inputs must be numeric.',
                    array('status' => 422)
                );
            }
...
SC_Lab_Microbiology_Laboratory_REST::boot();
