<?php
/**
 * Sustainable Catalyst Lab v0.26.3 cross-laboratory activation and runtime repair.
 */
if (!defined('ABSPATH')) { exit; }

final class SC_Lab_Runtime_Repair_V0263 {
    const VERSION = '0.26.3';
    const ROOT_ID = 'sc-lab-v0263-root';
    ...
    private static function asset_version($relative) {
        ...
        return self::VERSION . '.' . (is_file($path) ? (string) filemtime($path) : '0');
    }

    public static function maybe_enqueue() {
        if (!self::is_lab_request() || self::$assets) { return; }
        self::$assets = true;
        $style = 'assets/css/sc-lab-runtime-v0263.css';
        $script = 'assets/js/sc-lab-runtime-v0263.js';
        wp_enqueue_style('sc-lab-runtime-v0263', SC_LAB_URL . $style, array(), self::asset_version($style));
        ...
        wp_localize_script('sc-lab-runtime-v0263', 'SCLabRuntimeConfigV0263', array(
            ...
            'nodeBudget' => 6500,
            'warningBudget' => 5000,
            'healthUrl' => esc_url_raw(rest_url('sc-lab/v1/runtime/v0263/health')),
        ));
    }
    ...
    private static function single_module_shell($html, $requested) {
        ...
        return '<div id="' . esc_attr(self::ROOT_ID) . '" data-sc-lab-runtime="0.26.3" data-sc-lab-active-module="' . esc_attr($selected) . '" data-sc-lab-panel-total="' . count($panels) . '">' . self::bar($selected) . $body . '</div>';
    }
    ...
    private static function bar($module) {
        return '<aside class="sc-lab-runtime-bar-v0263" data-sc-lab-runtime-bar role="status"><div><strong>Calculator runtime repair active</strong><span>v0.26.3 · ' . esc_html($module) . ' · isolated laboratory activation</span></div><div><button type="button" data-sc-lab-runtime-action="overview">Overview</button><button type="button" data-sc-lab-runtime-action="reload">Reload laboratory</button><button type="button" data-sc-lab-runtime-action="diagnostics">Diagnostics</button></div><pre data-sc-lab-runtime-diagnostics hidden></pre></aside>';
    }
    ...
    public static function routes() {
        register_rest_route('sc-lab/v1', '/runtime/v0263/health', array(
    ...
    public static function health() {
        $asset_paths = array(
            ...
            'runtime' => 'assets/js/sc-lab-runtime-v0263.js',
        ...
        foreach ($asset_paths as $key => $relative) {
            ...
            $assets[$key] = array(
                'version' => self::asset_version($relative),
