<?php
/**
 * Comparative Economics and Development Systems REST proxy.
 *
...
final class SC_Lab_Comparative_Economics_Development_Systems_REST {
    ...
    public static function register_routes() {
        register_rest_route(
            'sc-lab/v1',
            '/compute/development-economics/methods',
            array(
                'methods' => 'GET',
        ...
        register_rest_route(
            'sc-lab/v1',
            '/compute/development-economics/run',
            array(
                'methods' => 'POST',
    ...
    private static function proxy(
        ...
        if ($backend === '') {
            return new WP_Error(
                'sc_lab_development_economics_backend_unavailable',
        ...
        if (!is_array($decoded)) {
            return new WP_Error(
                'sc_lab_development_economics_backend_response',
        ...
        if ($status < 200 || $status >= 300) {
            return new WP_Error(
                'sc_lab_development_economics_backend_error',
    ...
    public static function methods() {
        return self::proxy('/v1/development-economics/methods');
    }
    ...
    public static function run($request) {
        ...
        if (!is_array($body)) {
            return new WP_Error(
                'sc_lab_development_economics_invalid_payload',
        ...
        if ($method_id === '' || strpos($method_id, 'de.') !== 0) {
            return new WP_Error(
                'sc_lab_development_economics_invalid_method',
                'A methodId beginning with de. is required.',
                array('status' => 422)
            );
        }
        ...
        foreach ($raw_inputs as $key => $value) {
            if (!is_numeric($value)) {
                return new WP_Error(
                    'sc_lab_development_economics_invalid_input',
                    'Development-economics inputs must be numeric.',
                    array('status' => 422)
                );
            }
...
SC_Lab_Comparative_Economics_Development_Systems_REST::boot();
