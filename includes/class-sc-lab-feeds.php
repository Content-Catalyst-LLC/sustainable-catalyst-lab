...
class SC_Lab_Feeds {
    ...
    private static function remote_json($url, $args = array()) {
        $args = wp_parse_args($args, array('timeout'=>18, 'headers'=>array('Accept'=>'application/json','User-Agent'=>'SustainableCatalystLab/0.9.3 (+https://sustainablecatalyst.com/lab/)')));
    ...
    private static function arxiv($limit, $params) {
        ...
        $response = wp_safe_remote_get($url, array('timeout'=>18,'headers'=>array('User-Agent'=>'SustainableCatalystLab/0.9.3 (+https://sustainablecatalyst.com/lab/)')));
        if (is_wp_error($response)) { return $response; }
