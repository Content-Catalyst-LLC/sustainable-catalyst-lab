...
def test_migration_journal_is_verified_and_idempotent(tmp_path):
    ...
    plan = store.create_migration_plan({"id": "migration-one", "sourceVersion": "0.39.1", "targetVersion": "0.40.0"}, "alice")["migration"]
...
def test_contract_instances(tmp_path):
    ...
    migration = store.create_migration_plan({"id": "migration-contract", "sourceVersion": "0.39.1", "targetVersion": "0.40.0"}, "alice")["migration"]
