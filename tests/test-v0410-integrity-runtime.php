...
function get_option($name, $default = false) {
    if ($name === 'active_plugins') return array('sustainable-catalyst-lab/sustainable-catalyst-lab.php');
    return $default;
}
...
$checks = array(
    ...
    'release version is 0.41.0' => ($health['versions']['release'] ?? null) === '0.41.0',
    'feature release is 0.41.0' => ($health['versions']['featureRelease'] ?? null) === '0.41.0',
    'plugin header is 0.41.0' => ($health['versions']['pluginHeader'] ?? null) === '0.41.0',
    'manifest release is 0.41.0' => ($health['versions']['manifestRelease'] ?? null) === '0.41.0',
    'platform compatibility is 1.0.0' => ($health['versions']['platformCompatibility'] ?? null) === '1.0.0',
    'internal compatibility constant is 1.0.0' => ($health['versions']['pluginConstant'] ?? null) === '1.0.0',
    'manifest platform is 1.0.0' => ($health['versions']['manifestPlatform'] ?? null) === '1.0.0',
    'release line independently consistent' => !empty($health['releaseVersionConsistent']),
    'platform line independently consistent' => !empty($health['platformVersionConsistent']),
    'manifest hashes verify' => !empty($health['manifest']['verification']['ok']),
    'single plugin candidate' => count($health['pluginCandidates'] ?? array()) === 1,
);
