...
required={x for x in paths if x.startswith('/v1/visual-research-narrative-findings')}; assert len(required)==56,len(required)
assert catalog()['block_type_count']==12 and catalog()['finding_state_count']==5 and catalog()['handoff_target_count']==4 and catalog()['citation_mode_count']==3
assert interpretation_boundaries_report()['narrative_is_scientific_claim'] is False
print(f'INFO: {len(paths)} FastAPI path routes registered')
print('PASS: fifty-six v0.135.7 Visual Research Narrative/Findings routes loaded')
PYTEST
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-plugin.php >/dev/null
php -l includes/class-sc-lab-visual-research-narrative-findings-v01357.php >/dev/null
php tests/test-v01352.php; php tests/test-v01353.php; php tests/test-v01354.php; php tests/test-v01355.php; php tests/test-v01356.php; php tests/test-v01357.php
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.135.7' and m['featureVersion']=='0.135.7' and m['visualResearchNarrativeFindingsVersion']=='0.135.7'
assert m['v01357RequiredRouteCount']==56 and m['v01357BlockTypeCount']==12 and m['v01357FindingStateCount']==5 and m['v01357HandoffTargetCount']==4 and m['v01357CitationModeCount']==3
...
print(f"PASS - v0.135.7 manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/runtime + {len(m['backendCriticalFiles'])} backend files)")
PYTEST
echo "PASS - Sustainable Catalyst Lab v0.135.7 Visual Research Narrative, Findings & Publication Handoff release gate"
