...
echo "==> v0.49.0 Lab ↔ Workbench Model Handoff & Shared Computational Contract regression gate"
...
node tests/test-v0490.js
php tests/test-v0490.php
php tests/test-v0490-integrity-runtime.php
...
php -l includes/class-sc-lab-contextual-navigation-v0483.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/shared_model_handoff.py backend/app/probabilistic_analysis.py backend/app/graph_studio.py backend/app/response_surfaces.py backend/app/dynamic_systems.py backend/app/model_diagnostics.py backend/app/equation_builder.py backend/app/model_studio.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/model-handoff/health','/v1/model-handoff/policies','/v1/model-handoff/models/normalize','/v1/model-handoff/outbound/workbench','/v1/model-handoff/inbound/workbench'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.49.0 FastAPI model-handoff routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.49.0';assert m['featureVersion']=='0.49.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.49.0 Lab ↔ Workbench Model Handoff & Shared Computational Contract release gate"
