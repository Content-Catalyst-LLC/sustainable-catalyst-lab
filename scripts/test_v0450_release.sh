...
echo "==> Dynamic Systems + ODE Parameter Estimation + modeling regression gate"
...
node tests/test-v0450.js
php tests/test-v0450.php
php tests/test-v0450-integrity-runtime.php
node --check assets/js/modules/scientific-visualization-engine-v0440.js
node --check assets/js/modules/model-studio-v0450.js
node --check assets/js/modules/numerical-visualization-studio.js
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-plugin.php >/dev/null
php -l includes/class-sc-lab-model-studio-v0450.php >/dev/null
php -l includes/class-sc-lab-python-compute-core-v0261.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/dynamic_systems.py backend/app/model_diagnostics.py backend/app/equation_builder.py backend/app/model_studio.py backend/app/main.py

PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" - <<'PY'
from fastapi.testclient import TestClient
from app.main import app
c=TestClient(app)
r=c.get('/v1/model-studio/dynamic-systems/health')
assert r.status_code==200 and r.json()['version']=='0.45.0', r.text
p={'system':{'title':'Release gate decay','states':[{'symbol':'X','initial':10}],'parameters':[{'symbol':'k','role':'estimated','value':0.4,'bounds':{'lower':0,'upper':2}}],'equations':[{'state':'X','rhs':'-k*X'}],'timeSpan':{'start':0,'end':2,'points':11}}}
r=c.post('/v1/model-studio/dynamic-systems/simulate',json=p)
assert r.status_code==200, r.text
assert r.json()['simulation']['rowCount']==11
print('PASS - FastAPI dynamic-system routes')
PY
...
assert m['releaseVersion']=='0.45.0'
assert m['featureVersion']=='0.45.0'
...
echo "PASS - Lab v0.45.0 Dynamic Systems, ODE Models & Parameter Estimation release gate"
