...
echo "==> v0.79.0 Linked Views, Faceting & Figure Composition"
node --check assets/js/modules/linked-views-v0790.js
node --check assets/js/modules/graph-studio-v0790.js
node tests/test-v0790.js
php tests/test-v0790.php
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-linked-views-v0790.php >/dev/null
...
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
 backend/tests/test_linked_views_v0790.py backend/tests/test_time_parameter_space_v0780.py backend/tests/test_scientific_scene_v0770.py backend/tests/test_large_data_visualization_v0760.py backend/tests/test_scientific_data_binding_v0750.py backend/tests/test_visualization_engine_v0740.py backend/tests/test_visualization_engine_v0730.py backend/tests/test_data_transformations_v0550.py backend/tests/test_graph_studio_v0470.py backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_scientific_workflow_composer_v0570.py backend/tests/test_model_studio_v0460.py backend/tests/test_preregistration_v0700.py
echo "==> FastAPI visualization route registration"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" - <<'PY2'
from app.main import app
paths={r.path for r in app.routes}
required={'/v1/visualization/v0790/health','/v1/visualization/v0790/policies','/v1/visualization/v0790/views/normalize','/v1/visualization/v0790/links/normalize','/v1/visualization/v0790/facets/normalize','/v1/visualization/v0790/facets/build','/v1/visualization/v0790/compositions/normalize','/v1/visualization/v0790/interactions/propagate','/v1/visualization/v0790/workspaces/build','/v1/visualization/v0780/health','/v1/visualization/v0770/health','/v1/visualization/v0760/health','/v1/visualization/v0750/health'}
missing=sorted(required-paths)
if missing: raise SystemExit('FAIL - missing FastAPI routes: '+', '.join(missing))
print('PASS - v0.79.0 FastAPI route topology')
PY2
echo "==> contract validation"
"$PYTHON_BIN" - <<'PY2'
from pathlib import Path
import json
names=['linked-view-v0790.schema.json','view-link-v0790.schema.json','facet-spec-v0790.schema.json','linked-figure-composition-v0790.schema.json','scientific-figure-v0790.schema.json','figure-workspace-v0790.schema.json','linked-views-policy-v0790.json']
for base in ('contracts','backend/contracts'):
 for name in names: json.loads(Path(base,name).read_text())
print('PASS - v0.79.0 JSON contracts')
PY2
...
assert m['releaseVersion']=='0.79.0' and m['featureVersion']=='0.79.0' and m['platformVersion']=='1.0.0'
...
print(f"PASS - v0.79.0 release manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/source + {len(m['backendCriticalFiles'])} backend files)")
PY2
echo "PASS - Lab v0.79.0 Linked Views, Faceting & Figure Composition release gate"
