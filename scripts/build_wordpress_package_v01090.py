...
def main():
 ...
 if m.get('releaseVersion')!='0.109.0': raise SystemExit(f"ERROR: expected releaseVersion 0.109.0, got {m.get('releaseVersion')!r}")
 if m.get('platformCoreMinimumVersion')!='3.0.0': raise SystemExit('ERROR: Platform Core minimum is not 3.0.0')
 if m.get('platformCoreVisualReasoningScientificSceneVersion')!='0.109.0': raise SystemExit('ERROR: visual-scene bridge version mismatch')
