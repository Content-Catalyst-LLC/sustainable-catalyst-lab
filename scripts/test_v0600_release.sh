...
php tests/test-v0600.php
php tests/test-v0600-integrity-runtime.php
...
"$PYTHON_BIN" -m py_compile backend/app/integrated_research_beta_v0600.py backend/app/scientific_audit_v0590.py backend/app/scientific_compute_hardening.py backend/app/scientific_workflow_composer.py backend/app/config.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
...
print('PASS - v0.60.0 FastAPI integrated research beta routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.60.0';assert m['featureVersion']=='0.60.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.60.0 Integrated Scientific Research Beta release gate"
