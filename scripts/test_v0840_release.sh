...
echo "==> v0.84.0 GPU Renderer Architecture"
node --check assets/js/modules/gpu-renderer-architecture-v0840.js
node --check assets/js/modules/graph-studio-v0840.js
node --check assets/js/modules/release-console-v0821.js
node tests/test-v0840.js
php tests/test-v0840.php
...
required={
'/v1/visualization/v0840/health','/v1/visualization/v0840/policies','/v1/visualization/v0840/renderers/registry','/v1/visualization/v0840/capabilities/normalize','/v1/visualization/v0840/renderers/negotiate','/v1/visualization/v0840/buffers/plan','/v1/visualization/v0840/shaders/normalize','/v1/visualization/v0840/picking/normalize','/v1/visualization/v0840/diagnostics/build','/v1/visualization/v0840/workspaces/build',
'/v1/visualization/v0830/health','/v1/visualization/v0820/health','/v1/visualization/v0810/health','/v1/visualization/v0800/health','/v1/visualization/v0790/health','/v1/visualization/v0780/health','/v1/visualization/v0770/health','/v1/visualization/v0760/health','/v1/visualization/v0750/health'}
missing=sorted(required-paths)
if missing: raise SystemExit('FAIL - missing FastAPI routes: '+', '.join(missing))
print('PASS - v0.84.0 FastAPI route topology')
PY2
...
import json
names=['renderer-registry-v0840.schema.json','renderer-negotiation-v0840.schema.json','gpu-buffer-plan-v0840.schema.json','shader-descriptor-v0840.schema.json','picking-contract-v0840.schema.json','gpu-renderer-workspace-v0840.schema.json','gpu-renderer-policy-v0840.json']
for base in ('contracts','backend/contracts'):
 for name in names: json.loads(Path(base,name).read_text())
print('PASS - v0.84.0 JSON contracts')
PY2

echo "==> canonical release identity + GPU architecture assertions + manifest integrity"
"$PYTHON_BIN" - <<'PY2'
from pathlib import Path
import hashlib,json,re
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.84.0' and m['featureVersion']=='0.84.0' and m['platformVersion']=='1.0.0'
main=Path('sustainable-catalyst-lab.php').read_text(); header=re.search(r'^ \* Version: ([0-9.]+)$',main,re.M).group(1); assert header=='0.84.0'
console=Path('assets/js/modules/release-console-v0821.js').read_text(); assert "visualization/v0840/health" in console
browser=Path('assets/js/modules/gpu-renderer-architecture-v0840.js').read_text(); assert "webgl2ProductionRendererReady:false" in browser and "webgpuProductionRendererReady:false" in browser
backend=Path('backend/app/gpu_renderer_architecture_v0840.py').read_text(); assert '"webgl2ProductionRendererReady": False' in backend and '"webgpuProductionRendererReady": False' in backend
...
print(f"PASS - v0.84.0 release manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/source + {len(m['backendCriticalFiles'])} backend files)")
PY2

echo "PASS - Lab v0.84.0 GPU Renderer Architecture release gate"
