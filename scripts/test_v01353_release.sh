...
required={x for x in paths if x.startswith('/v1/multi-view-scientific-analysis-canvas')}
assert len(required)==41,len(required)
assert catalog()['panel_type_count']==10 and catalog()['layout_mode_count']==5 and catalog()['interaction_channel_count']==8
assert catalog()['filters_mutate_source_data'] is False and catalog()['automatic_join_inference'] is False
assert interpretation_boundaries_report()['determine_truth'] is False
print(f'INFO: {len(paths)} FastAPI path routes registered')
print('PASS: forty-one v0.135.3 Multi-View Scientific Analysis Canvas routes loaded')
print('PASS: canvas composition, linked selections, filters, synchronized context, snapshots, handoffs, and Core fixtures')
print('PASS: no scientific-record mutation, inferred joins, automatic validity/evidence/probability interpretation, truth determination, or Core submission')
PYTEST
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import json
for name in ['multi-view-scientific-analysis-canvas-v01353.schema.json','multi-view-scientific-analysis-canvas-policy-v01353.json']:
 a=Path('contracts',name).read_bytes(); b=Path('backend/contracts',name).read_bytes(); json.loads(a); json.loads(b); assert a==b,name
print('PASS - v0.135.3 contracts parse and mirror byte-for-byte')
PYTEST
...
assert m['releaseVersion']=='0.135.3' and m['featureVersion']=='0.135.3'
assert m['multiViewScientificAnalysisCanvasVersion']=='0.135.3'
assert m['v01353RequiredRouteCount']==41 and m['v01353PanelTypeCount']==10 and m['v01353LayoutModeCount']==5 and m['v01353InteractionChannelCount']==8
assert re.search(r'^ \* Version: 0\.135\.3$',Path('sustainable-catalyst-lab.php').read_text(),re.M)
...
print(f"PASS - v0.135.3 manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/runtime + {len(m['backendCriticalFiles'])} backend files)")
PYTEST
echo "PASS - Sustainable Catalyst Lab v0.135.3 Multi-View Scientific Analysis Canvas release gate"
