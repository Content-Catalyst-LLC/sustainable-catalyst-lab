...
echo "==> v0.77.0 3D Scientific Scene Engine"
node --check assets/js/modules/scientific-scene-engine-v0770.js
node --check assets/js/modules/graph-studio-v0770.js
node tests/test-v0770.js
php tests/test-v0770.php
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-scientific-scene-v0770.php >/dev/null
...
required={
 '/v1/visualization/v0770/health','/v1/visualization/v0770/policies','/v1/visualization/v0770/cameras/normalize','/v1/visualization/v0770/scenes/normalize','/v1/visualization/v0770/scenes/from-dataset','/v1/visualization/v0770/figures/build','/v1/visualization/v0770/workspaces/build',
 '/v1/visualization/v0760/health','/v1/visualization/v0750/health','/v1/visualization/v0740/health','/v1/visualization/v0730/health','/v1/graph-studio/health'
}
missing=sorted(required-paths)
if missing: raise SystemExit('FAIL - missing FastAPI routes: '+', '.join(missing))
print('PASS - v0.77.0 FastAPI route topology')
PY
echo "==> contract validation"
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import json
names=['scientific-scene-v0770.schema.json','scientific-scene-camera-v0770.schema.json','scientific-scene-object-v0770.schema.json','scientific-visualization-v0770.schema.json','scientific-figure-v0770.schema.json','figure-workspace-v0770.schema.json','scientific-scene-policy-v0770.json']
for base in ('contracts','backend/contracts'):
 for name in names: json.loads(Path(base,name).read_text())
print('PASS - v0.77.0 JSON contracts')
PY
...
assert m['releaseVersion']=='0.77.0' and m['featureVersion']=='0.77.0' and m['platformVersion']=='1.0.0'
...
print(f"PASS - v0.77.0 release manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/source + {len(m['backendCriticalFiles'])} backend files)")
PY
echo "PASS - Lab v0.77.0 3D Scientific Scene Engine release gate"
