...
php tests/test-v0580.php
php tests/test-v0580-integrity-runtime.php
...
"$PYTHON_BIN" -m py_compile backend/app/scientific_compute_hardening.py backend/app/scientific_workflow_composer.py backend/app/config.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in (
 '/v1/compute-hardening/v0580/health','/v1/compute-hardening/v0580/policies','/v1/compute-hardening/v0580/assess',
 '/v1/compute-hardening/v0580/dataset-window','/v1/compute-hardening/v0580/run','/v1/compute-hardening/v0580/jobs',
 '/v1/compute-hardening/v0580/jobs/{job_id}','/v1/compute-hardening/v0580/jobs/{job_id}/result',
 '/v1/compute-hardening/v0580/jobs/{job_id}/cancel','/v1/compute-hardening/v0580/cache','/v1/compute-hardening/v0580/cache/clear'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.58.0 FastAPI compute hardening routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.58.0';assert m['featureVersion']=='0.58.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.58.0 Large-Model, Large-Dataset & Compute Hardening release gate"
