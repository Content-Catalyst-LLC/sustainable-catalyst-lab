...
echo "==> Interactive Scientific Graph Engine + modeling regression gate"
...
node tests/test-v0440.js
php tests/test-v0440.php
php tests/test-v0440-integrity-runtime.php
node --check assets/js/modules/scientific-visualization-engine-v0440.js
node --check assets/js/modules/model-studio-v0440.js
node --check assets/js/modules/numerical-visualization-studio.js
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-plugin.php >/dev/null
php -l includes/class-sc-lab-model-studio-v0440.php >/dev/null
php -l includes/class-sc-lab-python-compute-core-v0261.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/model_diagnostics.py backend/app/equation_builder.py backend/app/model_studio.py backend/app/main.py

"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib, json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.44.0'
assert m['featureVersion']=='0.44.0'
...
echo "PASS - Lab v0.44.0 Interactive Scientific Graph Engine & Publication Graphics release gate"
