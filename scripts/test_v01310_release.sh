...
base='/v1/research-question-hypothesis-workspace'; required={p for p in paths if p.startswith(base)}; assert len(required)>=33,len(required)
assert hypothesis_registry({'hypotheses':[{'statement':'A'},{'statement':'B'}]})['automatic_hypothesis_selection'] is False
...
assert preregistration_lock({'content':{'x':1}})['preregistration']['locked'] is True
assert readiness_report({'checks':{k:1 for k in ['question','hypotheses','variables','observables','assumptions','expected_observations','falsification','scope']}})['ready_for_scientific_acceptance'] is False
assert core_object_plan({'question_refs':['q:1'],'hypothesis_refs':['h:1']})['automatic_core_submission'] is False
assert interpretation_boundaries_report()['determine_truth'] is False
print(f'INFO: {len(paths)} FastAPI path routes registered')
print('PASS: thirty-three v0.131 Research Question & Hypothesis Workspace routes loaded')
print('PASS: hypotheses, expectations, preregistration, readiness, and Core fixtures')
print('PASS: researcher selection retained; no automatic truth/validity/method/evidence/Core decisions')
PYTEST
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import json
for name in ['research-question-hypothesis-workspace-v01310.schema.json','research-question-hypothesis-workspace-policy-v01310.json']:
 a=Path('contracts',name).read_bytes(); b=Path('backend/contracts',name).read_bytes(); json.loads(a); json.loads(b); assert a==b,name
print('PASS - v0.131 contracts parse and mirror byte-for-byte')
PYTEST
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-plugin.php >/dev/null
php -l includes/class-sc-lab-research-question-hypothesis-workspace-v01310.php >/dev/null
php tests/test-v01310.php
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import hashlib,json,re
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.131.0' and m['featureVersion']=='0.131.0'
assert m['researchQuestionHypothesisWorkspaceVersion']=='0.131.0'
assert m['v01310RequiredRouteCount']==33 and m['v01310MethodFamilyCount']==20 and m['v01310FigureFamilyCount']==14
assert re.search(r'^ \* Version: 0\.131\.0$',Path('sustainable-catalyst-lab.php').read_text(),re.M)
...
print(f"PASS - v0.131.0 manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress runtime + {len(m['backendCriticalFiles'])} backend files)")
PYTEST
echo "PASS - Sustainable Catalyst Lab v0.131.0 Research Question & Hypothesis Workspace release gate"
