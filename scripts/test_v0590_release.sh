...
php tests/test-v0590.php
php tests/test-v0590-integrity-runtime.php
...
php -l includes/class-sc-lab-scientific-compute-hardening-v0580.php >/dev/null
php -l includes/class-sc-lab-scientific-audit-v0590.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/scientific_audit_v0590.py backend/app/security_privacy_hardening.py backend/app/scientific_compute_hardening.py backend/app/scientific_workflow_composer.py backend/app/config.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in (
 '/v1/scientific-audit/v0590/health','/v1/scientific-audit/v0590/policies','/v1/scientific-audit/v0590/scan',
 '/v1/scientific-audit/v0590/minimize','/v1/scientific-audit/v0590/redact-export','/v1/scientific-audit/v0590/reproducibility',
 '/v1/scientific-audit/v0590/audit','/v1/scientific-audit/v0590/verify'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.59.0 FastAPI scientific audit routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.59.0';assert m['featureVersion']=='0.59.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.59.0 R1 Scientific Audit Validation Dependency & Installer Repair release gate"
