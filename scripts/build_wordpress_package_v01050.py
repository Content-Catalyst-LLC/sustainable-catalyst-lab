...
def main():
 ...
 if m.get('releaseVersion')!='0.105.0': raise SystemExit(f"ERROR: expected releaseVersion 0.105.0, got {m.get('releaseVersion')!r}")
 if m.get('platformCoreRequiredVersion')!='3.0.0': raise SystemExit('ERROR: Platform Core requirement is not 3.0.0')
 if m.get('canonicalObjectMappingCount')!=20: raise SystemExit('ERROR: canonical object mapping count is not 20')
