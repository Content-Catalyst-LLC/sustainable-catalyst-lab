...
echo "==> v0.53.0 Correlated Uncertainty & Probabilistic Dependency Models regression gate"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
  backend/tests/test_correlated_uncertainty_v0530.py backend/tests/test_bayesian_inference_v0520.py backend/tests/test_advanced_statistical_modeling_v0510.py backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_shared_model_handoff_v0490.py \
...
node tests/test-v0530.js
php tests/test-v0530.php
php tests/test-v0530-integrity-runtime.php
...
php -l includes/class-sc-lab-correlated-uncertainty-v0530.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/correlated_uncertainty.py backend/app/probabilistic_analysis.py backend/app/bayesian_inference.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/model-studio/probabilistic/v0530/health','/v1/model-studio/probabilistic/v0530/policies','/v1/model-studio/probabilistic/v0530/normalize','/v1/model-studio/probabilistic/v0530/analyze','/v1/model-studio/probabilistic/v0530/estimate-dependency'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.53.0 FastAPI correlated uncertainty routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.53.0';assert m['featureVersion']=='0.53.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.53.0 Correlated Uncertainty & Probabilistic Dependency Models release gate"
