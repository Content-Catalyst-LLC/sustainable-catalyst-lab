...
def main():
 ...
 with zipfile.ZipFile(a.zip_path) as z:
  ...
  names=[n for n in z.namelist() if not n.endswith('/')]; m=json.loads(z.read(MANIFEST))
  if m.get('releaseVersion')!='0.126.0' or m.get('spatialSpatiotemporalResearchStudioVersion')!='0.126.0': raise SystemExit('ERROR: version mismatch')
