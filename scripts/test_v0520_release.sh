...
echo "==> v0.52.0 Bayesian Inference, Posterior Diagnostics & Posterior Predictive Modeling regression gate"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
  backend/tests/test_bayesian_inference_v0520.py backend/tests/test_advanced_statistical_modeling_v0510.py backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_shared_model_handoff_v0490.py \
...
node tests/test-v0520.js
php tests/test-v0520.php
php tests/test-v0520-integrity-runtime.php
node --check assets/js/modules/bayesian-inference-v0520.js
...
php -l includes/class-sc-lab-bayesian-inference-v0520.php >/dev/null
php -l includes/class-sc-lab-advanced-statistical-modeling-v0510.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/bayesian_inference.py backend/app/advanced_statistical_modeling.py backend/app/reproducible_model_package.py backend/app/shared_model_handoff.py backend/app/probabilistic_analysis.py backend/app/graph_studio.py backend/app/model_studio.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/model-studio/bayesian/health','/v1/model-studio/bayesian/policies','/v1/model-studio/bayesian/normalize','/v1/model-studio/bayesian/fit','/v1/model-studio/bayesian/posterior-predictive'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.52.0 FastAPI Bayesian routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.52.0';assert m['featureVersion']=='0.52.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.52.0 Bayesian Inference, Posterior Diagnostics & Posterior Predictive Modeling release gate"
