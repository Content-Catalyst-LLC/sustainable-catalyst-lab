...
php tests/test-v0601.php
php tests/test-v0601-integrity-runtime.php
node --check assets/js/modules/beta-field-diagnostics-v0601.js
node --check assets/js/modules/integrated-research-beta-v0600.js
node --check assets/js/modules/scientific-audit-v0590.js
node --check assets/js/modules/scientific-compute-hardening-v0580.js
...
php -l includes/class-sc-lab-beta-field-diagnostics-v0601.php >/dev/null
php -l includes/class-sc-lab-integrated-research-beta-v0600.php >/dev/null
php -l includes/class-sc-lab-scientific-audit-v0590.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/beta_field_diagnostics_v0601.py backend/app/integrated_research_beta_v0600.py backend/app/scientific_audit_v0590.py backend/app/scientific_compute_hardening.py backend/app/scientific_workflow_composer.py backend/app/config.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in (
 '/v1/beta-diagnostics/v0601/health','/v1/beta-diagnostics/v0601/policies','/v1/beta-diagnostics/v0601/scenarios',
 '/v1/beta-diagnostics/v0601/probe','/v1/beta-diagnostics/v0601/soak','/v1/beta-diagnostics/v0601/packet','/v1/beta-diagnostics/v0601/verify',
 '/v1/integrated-research/v0600/health','/v1/integrated-research/v0600/policies','/v1/integrated-research/v0600/capabilities',
 '/v1/integrated-research/v0600/journey','/v1/integrated-research/v0600/readiness','/v1/integrated-research/v0600/packet','/v1/integrated-research/v0600/verify'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.60.1 FastAPI beta diagnostics + v0.60.0 integrated beta routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.60.1';assert m['featureVersion']=='0.60.1';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.60.1 Beta Field Diagnostics, Integration Soak & Runtime Repair release gate"
