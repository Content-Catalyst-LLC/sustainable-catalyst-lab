...
def main():
 ...
 if m.get('releaseVersion')!='0.107.0': raise SystemExit(f"ERROR: expected releaseVersion 0.107.0, got {m.get('releaseVersion')!r}")
 if m.get('platformCoreMinimumVersion')!='3.0.0': raise SystemExit('ERROR: Platform Core minimum is not 3.0.0')
 if m.get('platformCoreScientificExecutionLineageVersion')!='0.107.0': raise SystemExit('ERROR: execution-lineage version mismatch')
