...
echo "==> v0.82.0 Uncertainty, Ensemble & Distribution Visualization"
node --check assets/js/modules/uncertainty-ensemble-distribution-v0820.js
node --check assets/js/modules/graph-studio-v0820.js
node tests/test-v0820.js
php tests/test-v0820.php
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-uncertainty-ensemble-distribution-v0820.php >/dev/null
...
paths={r.path for r in app.routes}; required={'/v1/visualization/v0820/health','/v1/visualization/v0820/policies','/v1/visualization/v0820/uncertainty/normalize','/v1/visualization/v0820/distributions/normalize','/v1/visualization/v0820/ensembles/normalize','/v1/visualization/v0820/layers/normalize','/v1/visualization/v0820/figures/attach','/v1/visualization/v0820/workspaces/build','/v1/visualization/v0810/health','/v1/visualization/v0800/health','/v1/visualization/v0790/health','/v1/visualization/v0780/health','/v1/visualization/v0770/health','/v1/visualization/v0760/health','/v1/visualization/v0750/health'}; missing=sorted(required-paths)
if missing: raise SystemExit('FAIL - missing FastAPI routes: '+', '.join(missing))
print('PASS - v0.82.0 FastAPI route topology')
PY2
echo "==> contract validation"
"$PYTHON_BIN" - <<'PY2'
from pathlib import Path
import json
names=['uncertainty-series-v0820.schema.json','empirical-distribution-v0820.schema.json','ensemble-visualization-v0820.schema.json','uncertainty-layer-v0820.schema.json','uncertainty-figure-v0820.schema.json','figure-workspace-v0820.schema.json','uncertainty-visualization-policy-v0820.json']
for base in ('contracts','backend/contracts'):
 for name in names: json.loads(Path(base,name).read_text())
print('PASS - v0.82.0 JSON contracts')
PY2
echo "==> release manifest integrity"
"$PYTHON_BIN" - <<'PY2'
from pathlib import Path
import hashlib,json,re
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text()); assert m['releaseVersion']=='0.82.0' and m['featureVersion']=='0.82.0' and m['platformVersion']=='1.0.0'
...
print(f"PASS - v0.82.0 release manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/source + {len(m['backendCriticalFiles'])} backend files)")
PY2
echo "PASS - Lab v0.82.0 Uncertainty, Ensemble & Distribution Visualization release gate"
