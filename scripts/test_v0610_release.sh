...
php tests/test-v0610.php
php tests/test-v0610-integrity-runtime.php
...
"$PYTHON_BIN" -m py_compile backend/app/scientific_study_lifecycle_v0610.py backend/app/beta_field_diagnostics_v0601.py backend/app/integrated_research_beta_v0600.py backend/app/scientific_audit_v0590.py backend/app/scientific_compute_hardening.py backend/app/scientific_workflow_composer.py backend/app/config.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in (
 '/v1/scientific-studies/v0610/health','/v1/scientific-studies/v0610/policies','/v1/scientific-studies/v0610/templates','/v1/scientific-studies/v0610/normalize',
 '/v1/scientific-studies/v0610/evaluate','/v1/scientific-studies/v0610/review','/v1/scientific-studies/v0610/packet','/v1/scientific-studies/v0610/verify',
 '/v1/beta-diagnostics/v0601/health','/v1/beta-diagnostics/v0601/probe','/v1/beta-diagnostics/v0601/soak',
 '/v1/integrated-research/v0600/health','/v1/integrated-research/v0600/journey'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.61.0 scientific-study + inherited beta routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.61.0';assert m['featureVersion']=='0.61.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.61.0 End-to-End Scientific Study & Research Project Lifecycle release gate"
