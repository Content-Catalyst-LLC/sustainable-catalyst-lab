...
php tests/test-v0650.php
php tests/test-v0650-integrity-runtime.php
...
print('PASS - v0.65.0 evidence grading + inherited synthesis/literature/claims/study/beta routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.65.0';assert m['featureVersion']=='0.65.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.65.0 Scientific Evidence Grading, Contradiction Analysis & Consensus Boundaries release gate"
