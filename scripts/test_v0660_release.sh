...
php tests/test-v0660.php
php tests/test-v0660-integrity-runtime.php
...
print('PASS - v0.66.0 argumentation + inherited evidence-grading/synthesis/literature/claims/study/beta routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.66.0';assert m['featureVersion']=='0.66.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.66.0 Competing Hypotheses & Scientific Argumentation release gate"
