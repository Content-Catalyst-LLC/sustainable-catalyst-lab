...
echo "==> v0.56.0 Advanced Experimental Design & Sequential Experimentation regression gate"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
  backend/tests/test_advanced_experimental_design_v0560.py backend/tests/test_data_transformations_v0550.py backend/tests/test_dynamic_systems_v0540.py backend/tests/test_correlated_uncertainty_v0530.py backend/tests/test_bayesian_inference_v0520.py backend/tests/test_advanced_statistical_modeling_v0510.py backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_shared_model_handoff_v0490.py \
...
php tests/test-v0560.php
php tests/test-v0560-integrity-runtime.php
...
"$PYTHON_BIN" -m py_compile backend/app/advanced_experimental_design.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/design-studies/v0560/health','/v1/design-studies/v0560/policies','/v1/design-studies/v0560/normalize','/v1/design-studies/v0560/optimal-design','/v1/design-studies/v0560/sequential-plan','/v1/design-studies/v0560/verify'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.56.0 FastAPI advanced experimental design routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.56.0';assert m['featureVersion']=='0.56.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.56.0 Advanced Experimental Design & Sequential Experimentation release gate"
