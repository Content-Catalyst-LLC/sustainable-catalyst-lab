...
def main():
 ...
 with zipfile.ZipFile(a.zip_path) as z:
  ...
  if m.get('releaseVersion')!='0.105.0': raise SystemExit('ERROR: releaseVersion is not 0.105.0')
  if m.get('platformCoreRequiredVersion')!='3.0.0': raise SystemExit('ERROR: stale Platform Core requirement')
  if m.get('canonicalObjectMappingCount')!=20: raise SystemExit('ERROR: canonical object mapping count mismatch')
  ...
  for rel,expected in sorted(critical.items()):
   ...
   if actual!=expected: raise SystemExit(f'ERROR: hash mismatch {rel}')
  if len(names)!=len(critical)+1: raise SystemExit(f'ERROR: file count {len(names)} != {len(critical)+1}')
  print('PASS: WordPress ZIP integrity')
  print('PASS: releaseVersion=0.105.0')
  print('PASS: Platform Core required release=3.0.0')
  print('PASS: canonicalObjectMappingCount=20')
