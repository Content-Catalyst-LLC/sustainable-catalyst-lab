...
echo "==> v0.50.0 Reproducible Model Packages, Registry & Research Bundles regression gate"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
  backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_shared_model_handoff_v0490.py \
...
node tests/test-v0500.js
php tests/test-v0500.php
php tests/test-v0500-integrity-runtime.php
...
"$PYTHON_BIN" -m py_compile backend/app/reproducible_model_package.py backend/app/shared_model_handoff.py backend/app/probabilistic_analysis.py backend/app/graph_studio.py backend/app/model_studio.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/model-packages/health','/v1/model-packages/policies','/v1/model-packages/build','/v1/model-packages/verify','/v1/model-packages/research-bundle','/v1/model-packages/registry-projection','/v1/model-packages/register'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.50.0 FastAPI model-package routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.50.0';assert m['featureVersion']=='0.50.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.50.0 Reproducible Model Packages, Registry & Research Bundles release gate"
