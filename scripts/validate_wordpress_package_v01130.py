...
def main():
 ...
 with zipfile.ZipFile(a.zip_path) as z:
  ...
  if m.get('releaseVersion')!='0.113.0': raise SystemExit('ERROR: releaseVersion is not 0.113.0')
  if m.get('platformCoreMinimumVersion')!='3.0.0': raise SystemExit('ERROR: stale Platform Core minimum requirement')
  if m.get('platformCoreProductionRuntimeVersion')!='0.113.0': raise SystemExit('ERROR: production runtime version mismatch')
  ...
  print('PASS: WordPress ZIP integrity'); print('PASS: releaseVersion=0.113.0'); print('PASS: Platform Core minimum release=3.0.0'); print('PASS: production runtime version=0.113.0'); print(f'PASS: verifiedFileCount={len(critical)}')
