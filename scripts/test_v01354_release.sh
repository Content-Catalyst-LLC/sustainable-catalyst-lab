...
required={x for x in paths if x.startswith('/v1/interactive-scientific-scene-drilldown')}
assert len(required)==45,len(required)
...
assert catalog()['automatic_hierarchy_inference'] is False and catalog()['automatic_join_inference'] is False
assert interpretation_boundaries_report()['determine_truth'] is False
print(f'INFO: {len(paths)} FastAPI path routes registered')
print('PASS: forty-five v0.135.4 Interactive Scientific Scene & Visual Drill-Down routes loaded')
print('PASS: scenes, explicit drill paths, mappings, navigation, LOD, snapshots, handoffs, and Core fixtures')
print('PASS: no scientific-record mutation, hierarchy/join inference, silent reprojection/interpolation, truth determination, or Core submission')
PYTEST
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import json
for name in ['interactive-scientific-scene-drilldown-v01354.schema.json','interactive-scientific-scene-drilldown-policy-v01354.json']:
 a=Path('contracts',name).read_bytes(); b=Path('backend/contracts',name).read_bytes(); json.loads(a); json.loads(b); assert a==b,name
print('PASS - v0.135.4 contracts parse and mirror byte-for-byte')
PYTEST
...
assert m['releaseVersion']=='0.135.4' and m['featureVersion']=='0.135.4'
assert m['interactiveScientificSceneDrillDownVersion']=='0.135.4'
assert m['v01354RequiredRouteCount']==45 and m['v01354LayerTypeCount']==12 and m['v01354DrillTargetTypeCount']==8
assert m['v01354ViewModeCount']==6 and m['v01354NavigationModeCount']==5 and m['v01354LodModeCount']==4
assert re.search(r'^ \* Version: 0\.135\.4$',Path('sustainable-catalyst-lab.php').read_text(),re.M)
...
print(f"PASS - v0.135.4 manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/runtime + {len(m['backendCriticalFiles'])} backend files)")
PYTEST
echo "PASS - Sustainable Catalyst Lab v0.135.4 Interactive Scientific Scene & Visual Drill-Down release gate"
