...
def main():
 ...
 if m.get('releaseVersion')!='0.108.0': raise SystemExit(f"ERROR: expected releaseVersion 0.108.0, got {m.get('releaseVersion')!r}")
 if m.get('platformCoreMinimumVersion')!='3.0.0': raise SystemExit('ERROR: Platform Core minimum is not 3.0.0')
 if m.get('platformCoreFindingsClaimsEvidenceValidationVersion')!='0.108.0': raise SystemExit('ERROR: findings/validation bridge version mismatch')
 ...
 print(f'PASS: packaged {len(critical)} manifest-critical WordPress files + canonical manifest')
