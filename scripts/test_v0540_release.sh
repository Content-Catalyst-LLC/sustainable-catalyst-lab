...
node tests/test-v0540.js
php tests/test-v0540.php
php tests/test-v0540-integrity-runtime.php
node --check assets/js/modules/dynamic-systems-v0540.js
...
php -l includes/class-sc-lab-dynamic-systems-v0540.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/dynamic_systems_v0540.py backend/app/dynamic_systems.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/model-studio/dynamic-systems/v0540/health','/v1/model-studio/dynamic-systems/v0540/policies','/v1/model-studio/dynamic-systems/v0540/normalize','/v1/model-studio/dynamic-systems/v0540/simulate','/v1/model-studio/dynamic-systems/v0540/bifurcation','/v1/model-studio/dynamic-systems/v0540/phase'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.54.0 FastAPI Dynamic Systems II routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.54.0';assert m['featureVersion']=='0.54.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.54.0 Dynamic Systems II release gate"
