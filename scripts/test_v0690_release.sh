...
php tests/test-v0690.php
php tests/test-v0690-integrity-runtime.php
...
print('PASS - v0.69.0 scientific theory + inherited modeling/evidence routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.69.0';assert m['featureVersion']=='0.69.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.69.0 Scientific Theory & Conceptual Model Workspace release gate"
