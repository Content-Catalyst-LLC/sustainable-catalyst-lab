...
paths={r.path for r in app.routes if isinstance(getattr(r,'path',None),str)};patch={p for p in paths if p.startswith('/v1/graph-studio-review-audit/v0135140')};assert len(patch)==13,len(patch)
print(f'PASS - manifest integrity ({len(m["wordpressCriticalFiles"])} WordPress/runtime + {len(m["backendCriticalFiles"])} backend files)')
print(f'PASS - FastAPI routes {len(paths)} total / {len(patch)} v0.135.14.0')
PY2
echo 'PASS - Lab v0.135.14.0 release gate'
