...
php tests/test-v0630.php
php tests/test-v0630-integrity-runtime.php
...
print('PASS - v0.63.0 literature provenance + inherited claims/study/beta routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.63.0';assert m['featureVersion']=='0.63.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.63.0 Scientific Literature, Citation Graph & Source-to-Claim Provenance release gate"
