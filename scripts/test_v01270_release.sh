...
echo "==> v0.127.0 time-series + spatial + causal + sensitivity + simulation + Bayesian + modeling + EDA + visualization + Core regression suite"
...
base='/v1/scientific-time-series-laboratory'; required={p for p in paths if p.startswith(base)}; assert len(required)>=31,len(required)
t=list(range(96)); y=[10+0.05*i+2*math.sin(2*math.pi*i/12)+0.2*math.sin(2*math.pi*i/5) for i in t]; p={'times':t,'values':y}
assert frequency_audit(p)['regular'] is True
assert len(decompose_series({**p,'period':12})['seasonal_pattern'])==12
assert len(autocorrelation({**p,'max_lag':12})['acf'])==13
assert fit_arima({**p,'p':2,'d':1,'q':1})['model_selected'] is False
assert len(forecast({**p,'model':'ar','model_spec':{'order':3},'horizon':6})['point_forecast'])==6
assert rolling_origin_evaluation({**p,'model':'ar','model_spec':{'order':2},'min_train':60,'step':8})['automatic_model_selection'] is False
...
print('PASS: thirty-one v0.127 Scientific Time-Series Laboratory routes loaded')
print('PASS: decomposition, dependence, ARIMA, forecasting, evaluation, spectral, snapshot, and Core fixtures')
print('PASS: no silent resampling/model selection/stationarity decisions, causal interpretation, scientific validity, or Core submission')
PYTEST
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import json
for name in ['scientific-time-series-laboratory-v01270.schema.json','scientific-time-series-laboratory-policy-v01270.json']:
 a=Path('contracts',name).read_bytes(); b=Path('backend/contracts',name).read_bytes(); json.loads(a); json.loads(b); assert a==b,name
print('PASS - v0.127 contracts parse and mirror byte-for-byte')
PYTEST
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-plugin.php >/dev/null
php -l includes/class-sc-lab-scientific-time-series-laboratory-v01270.php >/dev/null
php tests/test-v01270.php
"$PYTHON_BIN" - <<'PYTEST'
from pathlib import Path
import hashlib,json,re
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.127.0' and m['featureVersion']=='0.127.0'
assert m['scientificTimeSeriesLaboratoryVersion']=='0.127.0'
assert m['v01270RequiredRouteCount']==31 and m['v01270MethodFamilyCount']==15 and m['v01270FigureFamilyCount']==18
assert re.search(r'^ \* Version: 0\.127\.0$',Path('sustainable-catalyst-lab.php').read_text(),re.M)
...
print(f"PASS - v0.127.0 manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress runtime + {len(m['backendCriticalFiles'])} backend files)")
PYTEST
echo "PASS - Sustainable Catalyst Lab v0.127.0 Scientific Time-Series Laboratory release gate"
