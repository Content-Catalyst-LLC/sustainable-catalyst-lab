...
echo "==> v0.55.0 Scientific Data Transformation & Derived Variables regression gate"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
  backend/tests/test_data_transformations_v0550.py backend/tests/test_dynamic_systems_v0540.py backend/tests/test_correlated_uncertainty_v0530.py backend/tests/test_bayesian_inference_v0520.py backend/tests/test_advanced_statistical_modeling_v0510.py backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_shared_model_handoff_v0490.py \
...
php tests/test-v0550.php
php tests/test-v0550-integrity-runtime.php
node --check assets/js/modules/data-transformations-v0550.js
...
php -l includes/class-sc-lab-data-transformations-v0550.php >/dev/null
php -l templates/lab-app.php >/dev/null
"$PYTHON_BIN" -m py_compile backend/app/data_transformations.py backend/app/main.py
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
text=Path('backend/app/main.py').read_text()
for route in ('/v1/datasets/v0550/health','/v1/datasets/v0550/policies','/v1/datasets/v0550/normalize','/v1/datasets/v0550/transform','/v1/datasets/v0550/join'):
    assert route in text, f'missing FastAPI route: {route}'
print('PASS - v0.55.0 FastAPI scientific data transformation routes wired')
PY
"$PYTHON_BIN" - <<'PY'
from pathlib import Path
import hashlib,json
m=json.loads(Path('build/sc-lab-release-manifest.json').read_text())
assert m['releaseVersion']=='0.55.0';assert m['featureVersion']=='0.55.0';assert m['platformVersion']=='1.0.0'
errors=[]
...
PY
echo "PASS - Lab v0.55.0 Scientific Data Transformation & Derived Variables release gate"
