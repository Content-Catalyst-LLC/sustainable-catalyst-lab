...
echo "==> v0.81.0 Annotation, Measurement & Scientific Markup"
node --check assets/js/modules/scientific-markup-v0810.js
node --check assets/js/modules/graph-studio-v0810.js
node tests/test-v0810.js
php tests/test-v0810.php
php -l sustainable-catalyst-lab.php >/dev/null
php -l includes/class-sc-lab-annotation-measurement-markup-v0810.php >/dev/null
...
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" -m pytest -q \
 backend/tests/test_annotation_measurement_markup_v0810.py backend/tests/test_spatial_geospatial_raster_v0800.py backend/tests/test_linked_views_v0790.py backend/tests/test_time_parameter_space_v0780.py backend/tests/test_scientific_scene_v0770.py backend/tests/test_large_data_visualization_v0760.py backend/tests/test_scientific_data_binding_v0750.py backend/tests/test_visualization_engine_v0740.py backend/tests/test_visualization_engine_v0730.py backend/tests/test_data_transformations_v0550.py backend/tests/test_graph_studio_v0470.py backend/tests/test_reproducible_model_package_v0500.py backend/tests/test_scientific_workflow_composer_v0570.py backend/tests/test_model_studio_v0460.py backend/tests/test_preregistration_v0700.py
echo "==> FastAPI visualization route registration"
PYTHONPATH="$ROOT/backend${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" - <<'PY2'
from app.main import app
paths={r.path for r in app.routes}
required={'/v1/visualization/v0810/health','/v1/visualization/v0810/policies','/v1/visualization/v0810/annotations/normalize','/v1/visualization/v0810/measurements/compute','/v1/visualization/v0810/layers/normalize','/v1/visualization/v0810/figures/attach','/v1/visualization/v0810/workspaces/build','/v1/visualization/v0800/health','/v1/visualization/v0790/health','/v1/visualization/v0780/health','/v1/visualization/v0770/health','/v1/visualization/v0760/health','/v1/visualization/v0750/health'}
missing=sorted(required-paths)
if missing: raise SystemExit('FAIL - missing FastAPI routes: '+', '.join(missing))
print('PASS - v0.81.0 FastAPI route topology')
PY2
echo "==> contract validation"
"$PYTHON_BIN" - <<'PY2'
from pathlib import Path
import json
names=['scientific-annotation-v0810.schema.json','scientific-measurement-v0810.schema.json','scientific-markup-layer-v0810.schema.json','marked-scientific-figure-v0810.schema.json','figure-workspace-v0810.schema.json','scientific-markup-policy-v0810.json']
for base in ('contracts','backend/contracts'):
 for name in names: json.loads(Path(base,name).read_text())
print('PASS - v0.81.0 JSON contracts')
PY2
...
assert m['releaseVersion']=='0.81.0' and m['featureVersion']=='0.81.0' and m['platformVersion']=='1.0.0'
...
print(f"PASS - v0.81.0 release manifest integrity ({len(m['wordpressCriticalFiles'])} WordPress/source + {len(m['backendCriticalFiles'])} backend files)")
PY2
echo "PASS - Lab v0.81.0 Annotation, Measurement & Scientific Markup release gate"
