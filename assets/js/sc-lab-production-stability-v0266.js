(function (W, D) {
  ...
  const state = { version:'0.26.6', release:C.release || '0.26.6', module:C.requestedModule || 'overview', safeMode:!!C.safeMode, ready:false, backend:'unknown', backendFailures:0, backendRetryAttempt:0, backendLastError:null, restoredJobs:0, terminalJobs:0, baseline:null, last:null, max:null, startedAt:new Date().toISOString() };
  let banner = null;
  let sampler = null;
  ...
  function showBanner(kind, text) { const b=ensureBanner(); if(!b)return; b.hidden=false; b.dataset.kind=kind; const title=kind==='safe'?'Safe mode':kind==='compute'?'Compute reconnecting':'Lab recovery'; b.querySelector('[data-production-banner-title]').textContent=title; b.querySelector('[data-production-banner-message]').textContent=text; }
  ...
  function wrapComputeClient() {
    ...
    ['status','queueStatus','workers','runCore','execute','compare'].forEach(name=>{const original=api[name];if(typeof original!=='function')return;api[name]=async function(){try{const result=await original.apply(api,arguments);backendSuccess();return result;}catch(error){backendFailure(error,name);throw error;}};});
  ...
  function dispatchBackendState(){D.dispatchEvent(new CustomEvent('sc:lab:compute-state',{detail:{state:state.backend,failures:state.backendFailures,retryAttempt:state.backendRetryAttempt,lastError:state.backendLastError}}));}
  function backendSuccess(){state.backend='online';state.backendFailures=0;state.backendRetryAttempt=0;state.backendLastError=null;if(banner&&(banner.dataset.kind==='warning'||banner.dataset.kind==='compute'))banner.hidden=true;dispatchBackendState();}
  function backendFailure(error,operation){state.backend=navigator.onLine?'unavailable':'offline';state.backendFailures+=1;state.backendLastError=message(error);limitedPush(networkFailures,{operation,message:state.backendLastError,at:new Date().toISOString()},50);if(state.backend==='offline'){showBanner('warning','The browser is offline. Work remains local and queued jobs will resume when connectivity returns.');}else if(state.backendFailures<3){showBanner('compute',C.strings?.backendOffline||'Python Compute Core is reconnecting. Browser-local Lab tools remain available.');}else{showBanner('warning','Python Compute Core is still unavailable after repeated checks. Browser-local Lab tools remain available; retry compute or export diagnostics if this persists.');}dispatchBackendState();scheduleBackendRetry();}
  function scheduleBackendRetry(){if(backendRetry)return;const delays=[5000,10000,20000,40000,60000],delay=delays[Math.min(state.backendRetryAttempt,delays.length-1)];state.backendRetryAttempt+=1;backendRetry=W.setTimeout(()=>{backendRetry=null;checkBackend(false);},delay);}
  async function checkBackend(manual){const api=W.SCLab?.ComputeClient;if(!api?.isConfigured?.()){state.backend='not_configured';dispatchBackendState();return {ok:false,state:state.backend};}try{await api.status();backendSuccess();return {ok:true,state:'online'};}catch(error){backendFailure(error,manual?'manual_retry':'health_check');return {ok:false,state:state.backend,error:message(error)};}}
  ...
  function status(){const current=sample();return {version:'0.26.6',release:state.release,module:state.module,safeMode:state.safeMode,ready:state.ready,backend:state.backend,backendFailures:state.backendFailures,backendRetryAttempt:state.backendRetryAttempt,backendLastError:state.backendLastError,restoredJobs:state.restoredJobs,activeJobs:readJobs().length,current,baseline:state.baseline,max:state.max,heapGrowthBytes:heapGrowth(current),errors:errors.length,longTasks:longTasks.length,storage:W.SCLabProductionStorageV0266?.status?.()||null,budgets};}
