(function (w) {
  ...
  function blank(name = 'Untitled Lab Project') {
    const now = U.now();
    const project = { schemaVersion:'0.16.0', id:U.uid('project'), name, createdAt:now, updatedAt:now, description:'' };
  ...
  function normalize(project) {
    ...
    const merged = Object.assign(base, project || {}, { schemaVersion:'0.16.0' });
  ...
  class Projects {
    ...
    update(mutator,activity){const p=this.get();if(!p)return;mutator(p);p.schemaVersion='0.16.0';p.updatedAt=U.now();if(activity)p.activity.unshift({id:U.uid('activity'),at:p.updatedAt,text:activity});p.activity=p.activity.slice(0,750);this.save();}
...
// Project collection supported by v0.16.0: civilInfrastructureAnalyses

// Project collection supported by v0.16.0: infrastructureRecords

// Project collection supported by v0.16.0: infrastructureValidationRecords


// v0.16.0 project collections are created dynamically by the Architecture and Building Performance workspace:
...
// v0.16.0 project collections are created dynamically by the Urban Planning and Spatial Systems workspace:
...
// v0.16.0 project collections are created dynamically by Sustainable Cities and Urban Resilience:
...
// v0.16.0 project collections are created dynamically by Circular Economy and Industrial Ecology:
