(function (w) {
  ...
  const COLLECTIONS = [
    ...
    'visualizations','dimensionalScenes','chartExports','analysisPackets','methodContracts','codeArtifacts','implementationComparisons','codeExecutions','languageComparisons','runtimeRecords','compilerRecords','executionJobs','benchmarkRuns','crossLanguageValidationRecords'
  ];

  function blank(name = 'Untitled Lab Project') {
    const now = U.now();
    const project = { schemaVersion:'0.9.3', id:U.uid('project'), name, createdAt:now, updatedAt:now, description:'' };
  ...
  function normalize(project) {
    ...
    const merged = Object.assign(base, project || {}, { schemaVersion:'0.9.3' });
    COLLECTIONS.forEach(key => { if (!Array.isArray(merged[key])) merged[key] = []; });
    // Preserve legacy map records while exposing the v0.9.3 mapViews collection.
  ...
  class Projects {
    ...
    update(mutator,activity){const p=this.get();if(!p)return;mutator(p);p.schemaVersion='0.9.3';p.updatedAt=U.now();if(activity)p.activity.unshift({id:U.uid('activity'),at:p.updatedAt,text:activity});p.activity=p.activity.slice(0,750);this.save();}
