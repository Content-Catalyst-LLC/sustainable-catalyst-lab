(() => {
  ...
  const MODULE_ID = 'biomedical-engineering-biosignals';
  const ROOT_SELECTOR = '[data-biomedical-biosignals-root]';
  ...
  function core() {
    const candidate = Lab.BiomedicalEngineeringBiosignals;
  ...
  function chooseCanonicalRoot(panel) {
    ...
    if (!root) {
      root = allRoots.find(
        (candidate) => (
          candidate.querySelector('.sc-biosignal-shell')
        )
      ) || allRoots[0] || null;
    }
    ...
    for (const duplicate of allRoots) {
      ...
      const duplicateRendered = Boolean(
        duplicate.querySelector('.sc-biosignal-shell')
      );
  ...
  function clearStaleRoot(root) {
    ...
    const marker = root.dataset.scBiosignalVersion || '';
    ...
    if (
      ...
      delete root.dataset.scBiosignalVersion;
  ...
  function repair(reason = 'manual') {
    ...
    if (!engine) {
      state.lastError =
        'Biosignal v0.23.0 engine unavailable.';
      return false;
    }
    ...
    try {
      ...
      if (!rendered(root)) {
        state.lastError =
          'Biosignal interface did not render.';
        return false;
      }

      root.dataset.scBiosignalProductionVersion = VERSION;
      panel.dataset.scBiosignalProductionVersion = VERSION;
      state.repairs += 1;
  ...
  function health() {
    ...
    return {
      ...
      productionMarker:
        root?.dataset.scBiosignalProductionVersion || null,
      engineStatus,
      ...
      contractReady: Boolean(
        engine
        && engine?.definitions?.length === 48
        && engine?.benchmarks?.length === 48
        && engine?.categories?.length === 8
      ),
    };
  }
